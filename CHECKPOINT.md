...
**コードレビュー指摘P0修正完了・デザイン崩れ解消 → 次は GitHub 公開**
...
| ┗ Hugo deprecation解消 | `site.Data` → `hugo.Data`（affiliate-button shortcode） |
...
### STEP 1 — baseURL を確定してからGitHubへ push【最初にやる】

[site/config/_default/hugo.toml](site/config/_default/hugo.toml) を編集:

```toml
# 変更前
baseURL = "https://yourusername.github.io/amazon-affiliate-site/"

# 変更後（自分のGitHubユーザー名に）
baseURL = "https://lenovo-ueno.github.io/amazon-affiliate-site/"
```

[site/static/robots.txt](site/static/robots.txt) の Sitemap URLも同様に変更。

### STEP 2 — GitHubリポジトリを作成して push

```powershell
cd C:\Users\oueno\venv\ClaudeCodeWorkSpace\src\20_projects\amazon_affiliate_site
git init
git add .
git commit -m "initial commit"
git remote add origin https://github.com/[あなたのID]/amazon-affiliate-site.git
git push -u origin main
```

> ⚠️ `.env` は `.gitignore` に含まれているので push されない（APIキーは安全）

### STEP 3 — GitHub Pages の設定

リポジトリの **Settings → Pages → Source: GitHub Actions** を選択。

`deploy.yml` が自動的にサイトをビルド・公開する。
...
3. `git push` → 認証完了
4. サイトマップ送信: `https://lenovo-ueno.github.io/amazon-affiliate-site/sitemap.xml`
...
# → http://localhost:1313/amazon-affiliate-site/
...
```
Phase 1（今） → GitHub公開・必須ページ確認・GA4設定
...
│   ├── topic_registry.py      ← 50トピックのシードリスト
│   ├── trend_client.py        ← Google Trends + Gemini groundingでトピック取得（new）
...
│   ├── refresh_topics.py      ← トピック補充CLI（--add / --seeds / --source）（new）
...
└── .github/workflows/
    ...
    └── refresh_topics.yml     ← 日曜9時JST 自動トピック補充（new）
```
